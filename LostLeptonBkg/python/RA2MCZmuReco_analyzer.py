...
#InputFileName = "/afs/desy.de/user/a/adraeger/cmssm/Z-MC.root"
...
#### do the analyzing
process.MuReco= cms.EDAnalyzer("TagProbeFitTreeAnalyzer",
    # IO parameters:
    InputFileNames = cms.vstring(InputFileName),
    InputDirectoryName = cms.string("tapTreeMuId"),
    ...
    Categories = cms.PSet(
        MuIdPass = cms.vstring("MuIdPass", "dummy[pass=1,fail=0]"), 
...
##         probe_isCicHyperTight4 = cms.vstring("probe_isCicHyperTight4", "dummy[pass=1,fail=0]"),         
    ...
    PDFs = cms.PSet(
        ...
        gaussPlusLinear = cms.vstring(
            ...
            "RooExponential::backgroundPass(RecoMuInvariantMass, cPass[0,-2,2])",
            "RooExponential::backgroundFail(RecoMuInvariantMass, cFail[0,-2,2])",
            "efficiency[0.95,0,1]",
            "signalFractionInPassing[0.95]"
        ),
        gaussPlusQuadratic = cms.vstring(
             "Voigtian::signal(RecoMuInvariantMass, mean[91.2, 89.0, 93.0], width[2.4, 0.5, 5.0], sigma[5., 1., 12.0])",
             "RooExponential::backgroundPass(RecoMuInvariantMass, cPass[0,-2,2])",
#             "Chebychev::backgroundPass(mass, {cPass1[0,-2,2], cPass2[0,-2,2]})",
             "Chebychev::backgroundFail(RecoMuInvariantMass, {cFail1[0,-2,2], cFail2[0,-2,2]})",
             "efficiency[0.95,0,1]", 
             "signalFractionInPassing[0.95]"
        ),
        gaussPlusArgus = cms.vstring(
            "Voigtian::signal(RecoMuInvariantMass, mean[91.2, 89.0, 93.0], width[2.4, 0.5, 5.0], sigma[5., 1., 12.0])",
            "RooExponential::backgroundPass(RecoMuInvariantMass, cPass[0,-5,5])",
            "ArgusBG::backgroundFail(RecoMuInvariantMass, cFail1[86.6, 70, 100], cFail2[-2.8, -100, 100])",
            "efficiency[0.95,0,1]",
            "signalFractionInPassing[0.95]"
        ),
...
#            "RooExponential::backgroundPass(mass, cPass[0,-2,2])",
            "RooCMSShape::backgroundPass(RecoMuInvariantMass, alphaPass[60.,30.,90.], betaPass[0.001, 0.,0.1], betaPass, peakPass[90.0])",
